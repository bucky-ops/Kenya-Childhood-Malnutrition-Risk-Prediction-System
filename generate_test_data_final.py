...
def generate_test_data(output_dir="data/raw", num_months=24):
    ...
    # Save with the expected filename from config as CSV (since our system handles both)
    who_filename_csv = "Ch11-Nutrition-Figures.csv"  # Changed to CSV to match system expectations
    who_df.to_csv(Path(output_dir) / who_filename_csv, index=False)
    print(f"WHO Nutrition data saved: {len(who_df)} records as {who_filename_csv}")
    ...
    # Save with the expected filename from config as CSV
    unicef_filename_csv = "JMP_2021_INEQUALITIES_KEN_Kenya_1.csv"  # Changed to CSV
    unicef_df.to_csv(Path(output_dir) / unicef_filename_csv, index=False)
    print(f"UNICEF WASH data saved: {len(unicef_df)} records as {unicef_filename_csv}")
    ...
    # Save with the expected filename from config (it's already CSV)
    dhis2_filename = "metadata-nutrition-indicators-for-kenya.csv"  # Already CSV
    ...
    print(f"- {who_filename_csv} ({len(who_df)} records)")
    print(f"- {unicef_filename_csv} ({len(unicef_df)} records)")
...
def main():
    ...
    output_dir = "src/data/raw"  # Use the correct directory from config
    num_months = 12
