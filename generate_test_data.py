...
def generate_test_data(output_dir="data/raw", num_months=24):
    ...
    who_df.to_csv(Path(output_dir) / "who_nutrition.csv", index=False)
    print(f"WHO Nutrition data saved: {len(who_df)} records")
    
    # Generate UNICEF WASH Data
    ...
    unicef_df.to_csv(Path(output_dir) / "unicef_wash.csv", index=False)
    print(f"UNICEF WASH data saved: {len(unicef_df)} records")
    
    # Generate DHIS2 Cases Data
    ...
    dhis2_df.to_csv(Path(output_dir) / "dhis2_cases.csv", index=False)
    print(f"DHIS2 Cases data saved: {len(dhis2_df)} records")
    
    # Generate some additional contextual data
    ...
    print(f"- who_nutrition.csv ({len(who_df)} records)")
    print(f"- unicef_wash.csv ({len(unicef_df)} records)")
    print(f"- dhis2_cases.csv ({len(dhis2_df)} records)")
    print(f"- climate_data.csv ({len(climate_df)} records)")
    print(f"- economic_data.csv ({len(economic_df)} records)")
...
def main():
    ...
    output_dir = input("Enter output directory (default: data/raw): ").strip()
    if not output_dir:
        output_dir = "data/raw"
    
    num_months_input = input("Enter number of months of data to generate (default: 24): ").strip()
    try:
        num_months = int(num_months_input) if num_months_input else 24
    except ValueError:
        num_months = 24
        print(f"Invalid input, using default: {num_months} months")
    
    print(f"\nGenerating test data in '{output_dir}' with {num_months} months of data...")
